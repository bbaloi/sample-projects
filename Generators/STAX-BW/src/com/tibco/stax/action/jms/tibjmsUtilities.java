 ...
 *
 * $RCSfile: tibjmsUtilities.java,v $
 * $Revision: 1.19 $
 * $Date: 2004/02/05 18:36:22 $
 * =================================================================
 */
...
public class tibjmsUtilities
{
    ...
    public static void sslUsage()
    {
        ...
        System.err.println(" -ssl_renegotiate_size     <KBytes>    - renegotiate size in Kilobytes");
        System.err.println(" -ssl_renegotiate_interval <seconds>   - renegotiate interval in seconds");
        System.exit(0);
    }
...
private static class SSLParams implements com.tibco.tibjms.TibjmsSSLHostNameVerifier {
        ...
        int             ssl_renegotiate_size        = 0;
        int             ssl_renegotiate_interval    = 0;
        ...
        public SSLParams(String[] args) {
            ...
            while(i < args.length)
            {
                ...
                if (args[i].compareTo("-ssl_renegotiate_size")==0)
                {
                    if ((i+1) >= args.length) sslUsage();
                    ssl_renegotiate_size = Integer.parseInt(args[i+1]);
                    i += 2;
                }
                else
                if (args[i].compareTo("-ssl_renegotiate_interval")==0)
                {
                    if ((i+1) >= args.length) sslUsage();
                    ssl_renegotiate_interval = Integer.parseInt(args[i+1]);
                    i += 2;
                }
                else
        ...
        public void init() throws JMSSecurityException {
            ...
            com.tibco.tibjms.TibjmsSSL.setRenegotiateSize(ssl_renegotiate_size*1024);
            com.tibco.tibjms.TibjmsSSL.setRenegotiateInterval(ssl_renegotiate_interval*1000);
