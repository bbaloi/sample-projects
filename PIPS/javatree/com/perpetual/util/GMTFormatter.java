...
public class GMTFormatter extends SimpleDateFormat {

	private static final String FORMAT_PATTERN = "yyyyMMdd-HHmmss-S";
	
	public GMTFormatter() {
		super("yyyyMMdd-HHmmss-S", Locale.US);
		
		setTimeZone(TimeZone.getTimeZone("GMT"));
	}
	
	public String convertToGMT (Date date)
	{
		return format(date).toString();
	}
	
	public String convertToGMT (long timestamp)
	{
		return format(new Date(timestamp)).toString();
	}
	
	public long convertToMsFromEpoch (String gmtTimestamp)
	{	
		Date parsedDate = parse(gmtTimestamp, new ParsePosition(0));
