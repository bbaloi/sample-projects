...
public class LogRecordFormat implements java.io.Serializable
{
   ...
   private IPatternMatcher recordMatcherEngine=null;
        ...
        public LogRecordFormat(Element format) throws Exception
	{
		...
		for (Iterator i = m_fieldsElement.getChildren("field").iterator(); i.hasNext();)
		{
			...
			//	continue;
                        m_names.add(elem.getAttributeValue("name"));
                        //sLog.debug("ElementValue:"+elem.getAttributeValue("name"));
...
//		pattern = pattern.replaceAll("0", "\\\\d+").replaceAll("W", "\\\\S+").replaceAll(" ", "\\\\s+");
	...
	/*public Map parseLogRecord(String line) throws Exception
	{
		...
		return map;
         ...
         /*
         *Fields =              |Time|host|Sev|fac|Proc|PID|MSG|
         *SyslogRecord=     |id| date|host|sec|fac|date|host|proc|msg
         *SyslogRecordPID=  |id| date|host|sec|fac|date|host|proc|Pid|msg
         */
        public synchronized Map parseLogRecord(String line) throws Exception
        {
            Map map = new LinkedHashMap();
            IMessagePatternInfo patternInfo=recordMatcherEngine.findFirstMatch(line);
                ...
                if(patternInfo==null) 
                {
                    sLog.error("the pattern doesn't not match:");
                ...
                List childrenList = m_fieldsElement.getChildren();
                int fieldCount = childrenList.size();
                //sLog.debug("NumGroups:"+groupCount);
               // sLog.debug("Num fields:"+fieldCount);
                //String groupView="|";
                //for(int i=0;i<groupCount;i++)
                //{
                  //  groupView+=matcher.group(i)+"|";
                //}
                //sLog.debug("GroupView:"+groupView);
                
                //sLog.debug("Setting Date");
                Field dateField = new DateField((Element)childrenList.get(0), matcher.group(2));
                map.put(dateField.getName(), dateField);
                //sLog.debug("Setting Host");
                Field hostField = new TextField((Element)childrenList.get(1), matcher.group(3));
                map.put(hostField.getName(), hostField);
                //sLog.debug("Setting Sev");
                Field sevField = new MappedField((Element)childrenList.get(2), matcher.group(4));
                map.put(sevField.getName(), sevField);
                //sLog.debug("Setting Fac");
                Field facField = new MappedField((Element)childrenList.get(3), matcher.group(5));
                map.put(facField.getName(), facField);
                //sLog.debug("Setting Proc");
                Field procField = new TextField((Element)childrenList.get(4), matcher.group(6));
                map.put(procField.getName(), procField);                 
                           
                if(groupCount>fieldCount)
                {
                   // sLog.debug("Setting PID");
                    Field pidField = new TextField((Element)childrenList.get(5), matcher.group(8));
                    map.put(pidField.getName(), pidField);        
                    Field msgField = new TextField((Element)childrenList.get(6), matcher.group(9));
                    map.put(msgField.getName(), msgField);    
                }
                else
                {
                    //sLog.debug("Setting No PID");
                    Field noPidField = new TextField((Element)childrenList.get(5), "N/A");
                    map.put(noPidField.getName(), noPidField);  
                    Field msgField = new TextField((Element)childrenList.get(6), matcher.group(7));
                    map.put(msgField.getName(), msgField);    
                }
                       
                //sLog.debug("Setting Message");
		...
		Map map = new LinkedHashMap();
                ...
                //Iterator it =m_fieldsElement.getChildren().iterator();
                List childrenList = m_fieldsElement.getChildren();
		...
		//while(it.hasNext())
                for(int x=0;x<numChildren;x++)
	        {
                        ...
                        Element elem = (Element) childrenList.get(x);
                        String type = elem.getAttributeValue("type");
                        Field field;
                        String grpVal=matcher.group(j);
                        sLog.debug("Group-"+x+":"+grpVal);
                            ...
                            if(grpVal.equals(": "))
                            {
                                //elem = (Element)it.next();//this is the PID element
                                elem = (Element)childrenList.get(++x);
        ...
        */
	public abstract class Field implements Comparable,java.io.Serializable
	{
...
//System.out.println(toString() + " ? " + value);
	...
	public class TextField extends Field implements java.io.Serializable
	{
...
//System.out.println(m_time + " " + value);
	...
	public class DateField extends Field implements java.io.Serializable
	{
		private DateField(Element field, String text) throws Exception
		{
			...
			super(field);
                        try
                        {
                        //sLog.debug("Constructing New date field");
			m_format = new SimpleDateFormat(field.getAttributeValue("format"));
			//sLog.debug("constructed date field");
	...
	public class MappedField extends Field implements java.io.Serializable
	{
